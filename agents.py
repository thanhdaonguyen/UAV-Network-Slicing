...
class MADRLAgent:
    ...
    def __init__(self, 
        ...
        for i in range(num_agents):
            ...
            if self.device.type == 'cuda':
                self.actor_optimizers.append(
                    optim.AdamW(actor.parameters(), lr=self.lr_actor, fused=True)  # Fused for speed!
                )
            else:
                self.actor_optimizers.append(
                    optim.Adam(actor.parameters(), lr=self.lr_actor)
                )
        ...
        if self.device.type == 'cuda':
            self.critic_optimizer = optim.AdamW(
                self.critic.parameters(), lr=self.lr_critic, fused=True
            )
        else:
    ...
    def train_rl_with_bc_regularization(self, bc_weight):
        ...
        # ============================================
        # CRITICAL: Efficient Data Transfer
        # ============================================
        
        # Sample batch
        states, actions, greedy_actions, rewards, next_states, dones = \
            self.buffer.sample(self.batch_size)
        
        # Convert to tensors with pinned memory
        if self.pin_memory:
            states = torch.FloatTensor(states).pin_memory()
            actions = torch.FloatTensor(actions).pin_memory()
            greedy_actions = torch.FloatTensor(greedy_actions).pin_memory()
            rewards = torch.FloatTensor(rewards).pin_memory()
            next_states = torch.FloatTensor(next_states).pin_memory()
            dones = torch.FloatTensor(dones).pin_memory()
        else:
            states = torch.FloatTensor(states)
            actions = torch.FloatTensor(actions)
            greedy_actions = torch.FloatTensor(greedy_actions)
        ...
        greedy_actions = greedy_actions.to(self.device, non_blocking=True)
        rewards = rewards.to(self.device, non_blocking=True).unsqueeze(-1)
        next_states = next_states.to(self.device, non_blocking=True)
        dones = dones.to(self.device, non_blocking=True).unsqueeze(-1)
        ...
        with torch.no_grad():
            ...
            target_value = rewards + self.gamma * target_q
            target_value = torch.clamp(target_value, -500.0, 500.0)
        ...
        for i in range(self.num_agents):
            ...
            greedy_action = greedy_actions_per_agent[:, i, :]
            predicted_action = self.actors[i](states_per_agent[:, i, :])
            
            # RL loss
            q_values = self.critic(states, current_actions_flat)
            rl_loss_normalized = -q_values.mean() / 20000.0
            
            # BC losses
            position_bc = nn.functional.mse_loss(predicted_action[:, :3], greedy_action[:, :3])
            power_bc = nn.functional.mse_loss(predicted_action[:, 3:4], greedy_action[:, 3:4])
            bandwidth_bc = nn.functional.mse_loss(predicted_action[:, 4:], greedy_action[:, 4:])
            
            bc_loss = 0.3 * position_bc + 0.2 * power_bc + 0.5 * bandwidth_bc
            total_loss = (1 - bc_weight) * rl_loss_normalized + bc_weight * bc_loss
            
            self.actor_optimizers[i].zero_grad(set_to_none=True)
            total_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.actors[i].parameters(), 0.3)
            self.actor_optimizers[i].step()
